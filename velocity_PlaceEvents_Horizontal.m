...
%% 
idSubject       = [1]; % finished for idSubjects 1,4,6:9
idSignalEvent   = 10;
...
sTrialTypeList  = {'UP_HF','UP_HS','SIDE_HF','SIDE_HS'}; 
...
for iTrialType = 1:numel(sTrialTypeList)
 ...
 nData      = dbox.getSignal(idSignalListFCR(1),idTrial,'sScript',sScript);
            ...
            % FCR is on the underside of the arm. The x-acceleration, 
            % which should dominate the signal, is
            % positive when pointing nearly straight left. So, when FCR
            % acceleration is positive it's a leftward motion and when it's
            % negative it's a rightward motion. Therefore if the movement
            % begins with positive velocity, it's a leftward direction.
            ...
            startLeft = startLeft + nStart;
            startRight = startRight + nStart;
            ...
            tStartLeft = startLeft/nRate;
            tStartRight = startRight/nRate;
            ...
            plot(tStartLeft,highpassECU(startLeft-nStart),'ro')
            plot(tStartRight,highpassECU(startRight-nStart),'ko')
            legend('Velocity','Left Events','Right Events',Location='best') %'Acceleration',
            ylabel('ECU')
            hold off
            ...
            plot(tStartLeft,highpassFCU(startLeft-nStart),'ro')
            plot(tStartRight,highpassFCU(startRight-nStart),'ko')
            ...
            plot(tStartLeft,highpassFCR(startLeft-nStart),'ro')
            plot(tStartRight,highpassFCR(startRight-nStart),'ko')
            % legend('Velocity','Events',Location='best') %'Acceleration',
            ylabel('FCR')
            hold off
            xlabel('Time (s)')
            ...
            dbox.setEvent(tStartLeft,idTrial,idSignalEvent,1);
            dbox.setEvent(tStartRight,idTrial,idSignalEvent,2);
